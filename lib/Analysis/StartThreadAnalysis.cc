...
bool StartThreadAnalysis::isInLocalFile(clang::SourceRange range) {
  ...
  return (n < 3 || (name[n-1] == 'c' && name[n-2] == 'c' && name[n-3] == '.'));
  // return (name[0] != '/');
}
...
bool StartThreadAnalysis::isThreadType(clang::QualType qt)
{
  ...
  if (const clang::ElaboratedType* ety = llvm::dyn_cast<clang::ElaboratedType>(ty)) {
    sl::l() << "removing elaboration...\n";
    return isThreadType(ety->getNamedType());
  } else if (const clang::RecordType* rty = llvm::dyn_cast<clang::RecordType>(ty)) {
...
bool StartThreadAnalysis::TraverseFunctionDecl(clang::FunctionDecl *func) {
  if (isInLocalFile(func->getSourceRange())) {
    ...
    l() << "TraverseFunctionDecl " << func->getNameAsString() << " @Starts nothing? " << contextStartsNothing << "\n"; 
    bool res = clang::RecursiveASTVisitor<StartThreadAnalysis>::TraverseFunctionDecl(func);
    contextStartsNothing = savedDeclaredStarts;
    return res;
  } else {
    return clang::RecursiveASTVisitor<StartThreadAnalysis>::TraverseFunctionDecl(func);
  }
}
...
bool StartThreadAnalysis::VisitCallExpr(clang::CallExpr *c) {
  if (isInLocalFile(c->getSourceRange())) 
	...
	if (contextStartsNothing && !declaresStartsNothing(func)) {
	  l() << "!!! Found unsafe call to " << func->getNameAsString() << " that may start a thread\n";
	}
      }
    }
  return true;
...
bool StartThreadAnalysis::VisitVarDecl(clang::VarDecl *decl) {
  if (isInLocalFile(decl->getSourceRange())) {
	...
	if (isThreadType(t) && contextStartsNothing) {
	  sl::l() << "!!!! Found thread " << decl->getNameAsString() << " in code that is not supposed to have them\n";
	}
      }
  }
  return true;
